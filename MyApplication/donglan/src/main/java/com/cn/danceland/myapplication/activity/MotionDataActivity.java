...
public class MotionDataActivity extends BaseActivity {
    ...
    public void find_all_data(final int pageCount) throws JSONException {
        String url;//有氧
        if (isAerobic) {
            url = Constants.QUERY_SH_AEROBIC;//有氧
        } else {
            url = Constants.QUERY_SH_WEIGHT;//无氧
        }
...
//        MyStringRequest stringRequest = new MyStringRequest(Request.Method.GET, Constants.QUERY_USER_DYN_INFO_URL + myInfo.getPerson().getId(), new Response.Listener<String>() {
//            @Override
//            public void onResponse(String s) {
